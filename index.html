...
.preset-search-btn { padding: 8px 10px; background: var(--bg); border: 1px solid var(--border); border-radius: var(--radius-sm); font-size: 12px; font-weight: 600; color: var(--text-sub); cursor: pointer; white-space: nowrap; flex-shrink: 0; }
...
.op-select, .row-op-select { padding: 5px 4px; border: 1px solid var(--border); border-radius: var(--radius-sm); font-size: 11px; font-weight: 700; color: var(--green-dark); background: var(--green-light); cursor: pointer; flex-shrink: 0; -webkit-appearance: none; text-align: center; }
...
.modal-open-btn { display: flex; align-items: center; gap: 4px; padding: 6px 12px; border: 1px solid var(--border); border-radius: 20px; background: none; font-size: 12px; color: var(--text-sub); cursor: pointer; white-space: nowrap; }
...
<div class="search-panel">
  ...
  <div class="direct-search-row">
    <input type="text" id="keyword" placeholder="직접 검색..." autocomplete="off" onkeydown="if(event.key==='Enter') doDirectSearch()">
...
(function init() {
  ...
  if (id && secret) document.getElementById('apiDot').classList.add('active');
...
async function searchAll() {
  const queries = [];
  [0,1,2].forEach(i => { buildQueries(i).forEach(q => { if (q && !queries.includes(q)) queries.push(q); }); });
  if (!queries.length) { showToast('키워드를 입력해주세요.'); return; }
  await runSearch(queries);
}
  ...
  try {
    ...
    for (const query of queries) {
      ...
      (data.items || []).forEach(item => {
        ...
        if (!seenLinks.has(link)) { seenLinks.add(link); allItems.push(item); }
      });
    }
    articles = allItems
...
async function shareOne(id) {
  const a = articles[id];
  if (navigator.share) {
    try { await navigator.share({ title: a.title, url: a.link }); } catch(e) {}
  } else {
    const text = `📰 ${a.title}\n${a.link}`;
